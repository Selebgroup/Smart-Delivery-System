...
unsigned long lastAlarmDurationUpdate = 0;
unsigned long lastAlarmDurationSeconds = 0;
...
const int MAX_OFFLINE_ALARM_EVENTS = 30;
AlarmEvent offlineAlarmEvents[MAX_OFFLINE_ALARM_EVENTS];
int offlineAlarmEventCount = 0;
...
const int TIMEZONE_OFFSET_HOURS = 7;
...
void startHighTempAlarm();
void stopHighTempAlarm();
void updateAlarmDuration();
void flushOfflineAlarmEvents();
...
void syncRTCFromGPS() {
  ...
  int year = gpsDate.substring(4, 6).toInt() + 2000;
  ...
  DateTime localTime = utcTime + TimeSpan(TIMEZONE_OFFSET_HOURS * 3600L);
  rtc.adjust(localTime);
  rtcSyncedFromGPS = true;
  SerialMon.print("RTC synced from GPS to: ");
  SerialMon.print(localTime.year());
  SerialMon.print("-");
  SerialMon.print(localTime.month());
  SerialMon.print("-");
  SerialMon.print(localTime.day());
  SerialMon.print(" ");
  SerialMon.print(localTime.hour());
  SerialMon.print(":");
  SerialMon.print(localTime.minute());
  SerialMon.print(":");
  SerialMon.println(localTime.second());
}
...
void checkBuzzer(float temperature) {
  static bool wasAboveSetpoint = false;
  bool isAboveSetpoint = temperature >= tempSetpoint;
...
void publishAlarmNyala(const String& timeStr) {
  if (!mqtt_client.connected()) {
    return;
  }
...
void publishAlarmDuration(unsigned long seconds) {
  if (!mqtt_client.connected()) {
    return;
  }
  StaticJsonDocument<128> doc;
...
void publishAlarmStopWithDuration(const String& timeStr, unsigned long seconds) {
  if (!mqtt_client.connected()) {
    return;
  }
  StaticJsonDocument<128> doc;
...
void storeOfflineAlarmNyala(const String& timeStr) {
  if (offlineAlarmEventCount >= MAX_OFFLINE_ALARM_EVENTS) {
    return;
  }
  AlarmEvent &event = offlineAlarmEvents[offlineAlarmEventCount++];
  event.type = 0;
  strncpy(event.timeStr, timeStr.c_str(), sizeof(event.timeStr));
  event.timeStr[sizeof(event.timeStr) - 1] = '\0';
  event.durationSeconds = 0;
}

void storeOfflineAlarmMati(const String& timeStr, unsigned long seconds) {
  if (offlineAlarmEventCount >= MAX_OFFLINE_ALARM_EVENTS) {
    return;
  }
  AlarmEvent &event = offlineAlarmEvents[offlineAlarmEventCount++];
...
void startHighTempAlarm() {
  ...
  lastAlarmDurationUpdate = millis();
  lastAlarmDurationSeconds = 0;
  alarmStartTimeStr = getCurrentTimeString();
  alarmEndTimeStr = "";
  if (mqtt_client.connected()) {
    publishAlarmNyala(alarmStartTimeStr);
  } else {
    storeOfflineAlarmNyala(alarmStartTimeStr);
  }
}
...
void stopHighTempAlarm() {
  ...
  unsigned long elapsedSeconds = (millis() - alarmStartMillis) / 1000;
  if (elapsedSeconds > lastAlarmDurationSeconds) {
    lastAlarmDurationSeconds = elapsedSeconds;
  }
  alarmEndTimeStr = getCurrentTimeString();
  if (mqtt_client.connected()) {
    publishAlarmStopWithDuration(alarmEndTimeStr, lastAlarmDurationSeconds);
  } else {
    storeOfflineAlarmMati(alarmEndTimeStr, lastAlarmDurationSeconds);
  }
  alarmState = ALARM_IDLE;
  alarmStartMillis = 0;
  lastAlarmDurationUpdate = 0;
}
...
void updateAlarmDuration() {
  ...
  if (nowMillis - lastAlarmDurationUpdate < fastPublishInterval) {
    return;
  }
  unsigned long elapsedSeconds = (nowMillis - alarmStartMillis) / 1000;
  if (elapsedSeconds == 0 || elapsedSeconds == lastAlarmDurationSeconds) {
    return;
  }
  lastAlarmDurationSeconds = elapsedSeconds;
  lastAlarmDurationUpdate = nowMillis;
  if (mqtt_client.connected()) {
    publishAlarmDuration(lastAlarmDurationSeconds);
  }
}
...
void flushOfflineAlarmEvents() {
  ...
  for (int i = 0; i < offlineAlarmEventCount; i++) {
    AlarmEvent &event = offlineAlarmEvents[i];
    if (event.type == 0) {
      publishAlarmNyala(String(event.timeStr));
    } else if (event.type == 1) {
      publishAlarmStopWithDuration(String(event.timeStr), event.durationSeconds);
    }
  }
  offlineAlarmEventCount = 0;
}
